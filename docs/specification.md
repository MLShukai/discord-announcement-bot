...
1. **初回告知（日曜 12:00・自動）** — 公開告知チャンネルへ「今週の予定」を投稿し、確認チャンネルへ報告する。
   - 公開告知チャンネルへ種別ごとのテンプレートで本文を投稿する（**リアクション・凡例は付けない**）。投稿メッセージIDを状態に保存する（再投稿時の削除対象特定のため）。
   - 確認チャンネル（運営用）へ報告メッセージを投稿する。`action_role`（告知管理者）を @ メンションし、現在の種別とリアクション凡例を付ける。👍 ⚡ 🛠️ 💤 の 4 リアクションを付与し、メッセージIDを状態に保存する（再起動後もリアクション追従するため）。
   - **新しい開催週**に入った場合は、投稿前に種別を通常開催へリセットし LT情報をクリアする。
2. **種別の変更（日曜〜開催日）** — 次のいずれかで今週の種別を変更できる。変更すると公開告知を**削除して新しい種別で再投稿**する。
   - 確認チャンネルの報告メッセージへの**リアクション**（admin/moderator ロールのみ有効、それ以外は無視）。公開告知チャンネルのメッセージには種別変更リアクションは付かない。
   - スラッシュコマンド `/plan set <種別>`。
3. **開催告知（開催時・手動 `/open`）** — 公開告知チャンネルへ開催告知を投稿する。
...
- **`/config`** — 設定管理（admin/moderator）
  ...
  - `/config role [role]`、`/config channel announce [channel]`、`/config channel confirm [channel]`
...
- 確認チャンネルの報告メッセージへのリアクションによる種別変更: admin または moderator（他は無視）
...
- **実行時状態** (`state.json`): 今週の種別・LT情報・公開告知メッセージID・確認報告メッセージID・
  対象開催日を JSON で永続化。`StateStore` が変更のたびに保存。再起動で復元。
...
- **`AnnounceBotClient`** (`src/bot/client.py`): composition root。`Clock` / `ConfigManager` /
  ...
  初回告知タスクを 1 本登録、コマンドツリーを sync。`on_raw_reaction_add` で確認報告メッセージ
  （保存済みID一致）へのリアクションを判定し、権限ガード後に種別を更新・永続化して公開告知を再投稿する。
...
- **`AnnouncementService`** (`src/bot/announcement/`): `Clock`・config・state を注入。
  `next_event_date()` / `build_announce(type)` / `build_confirm(type)` /
  `build_open(type)->str|None`（REST は None）/ `send_announce(channel)`（公開告知・リアクションなし）/
  `send_confirm(channel, mention)`（確認報告・4 リアクション付与）/ `reannounce(channel)`（旧告知削除→再投稿）/
  `send_open(channel)`。`string.Template` で文面を展開する。
...
[channels]
announce_channel_id = ""   # 公開告知チャンネル
confirm_channel_id = ""    # 確認チャンネル (運営用)
...
```json
{
  ...
  "confirm_message_id": 1234567890987654321,
